...
estimate (~`4 parent_id + 4 subtree_end + 4 counts + len(name)` bytes
per dir plus map overhead; low-GB for 10-100M dirs) and report the
measured figure. Covers all 4 acceptance tests from I1 (flag off ->
identical to ClickHouse-only path; flag on + built -> results match
with no ClickHouse round-trip, verified by query-count assertion;
flag on + not yet built -> transparent fallback returning correct
results; benchmark reports memory/build cost and per-query latency
delta vs the ClickHouse path). Depends on Phases 1-2 (catalog) and
the navigation query paths from Phase 4; benchmark hooks from Phase
7.
